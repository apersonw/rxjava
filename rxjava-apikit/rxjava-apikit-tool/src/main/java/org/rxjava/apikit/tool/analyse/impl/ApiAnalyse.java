...
public class ApiAnalyse {
     ...
     * 扫描待分析包下的类并开始分析
     */
    public void analyse(String analysePackage) {
        newContext = new ApiContext();
        newContext.setAnalysePackage(analysePackage);
        ...
        scanner.matchAllClasses(this::analyseController);
        scanner.scan();
    }
     ...
     * 一、分析控制器
     */
    private void analyseController(Class cls) {
        //仅分析指定包下类
        String classPackageName = cls.getPackage().getName();
        if (!classPackageName.startsWith(newContext.getAnalysePackage())) {
            return;
        }
